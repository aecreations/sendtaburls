pref("extensions.{4aebcd37-f454-4928-9233-174a026ed367}.description", "chrome://sendtabs/locale/sendtabs.properties");

pref("extensions.aecreations.sendtabs.message.format.list_style", 0);
pref("extensions.aecreations.sendtabs.message.format.always_ask", true);
pref("extensions.aecreations.sendtabs.mailclient", 0);
pref("extensions.aecreations.sendtabs.mailclient.googleapps.enabled", false);
pref("extensions.aecreations.sendtabs.mailclient.googleapps.domain", "");
pref("extensions.aecreations.sendtabs.mailclient.clipboard.url_count_position", 1);
pref("extensions.aecreations.sendtabs.mailclient.clipboard.confirm_copy", true);
pref("extensions.aecreations.sendtabs.webmail.open_in_new_window", false);
pref("extensions.aecreations.sendtabs.webmail.window_features", "");
pref("extensions.aecreations.sendtabs.show_tab_menu_command", true);
pref("extensions.aecreations.sendtabs.reload_pending_tabs", true);
pref("extensions.aecreations.sendtabs.migrate_prefs", true);
